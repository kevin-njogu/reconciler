...
from app.utility_functions.constant_variables import UNRECONCILED, RECONCILED, DEPOSITS, CHARGES
...
def get_mpesa_data(session_key: str, configs:dict) -> pd.DataFrame:
    try:
        utility_df = pd.DataFrame()
        mmf_df = pd.DataFrame()
        for key in ["util_prefix", "mmf_prefix"]:
            df = read_file(
                session_id=session_key,
                filename_prefix=configs.get(key),
        ...
        final_df = pd.concat([utility_df, mmf_df], ignore_index=True)
        return final_df
    except Exception:
        raise


def clean_mpesa_data(session_key: str, configs: dict,  gateway_columns:dict) -> pd.DataFrame:
    try:
        ...
        df = get_mpesa_data(session_key, configs)
...
def get_mpesa_debits(df: pd.DataFrame, details_col: str, debits_col: str, reference_col: str) -> pd.DataFrame:
    try:
        charges_keys = ["Business Pay Bill Charge", "Business Buy Goods Charge", "B2C Payment Charge"]
...
def get_mpesa_credits(df: pd.DataFrame, credits_col: str, remarks_col: str) -> pd.DataFrame:
    try:
        dataframe = df
...
def get_mpesa_charges(df:pd.DataFrame, details_col: str, credits_col:str, remarks_col:str) -> pd.DataFrame:
    try:
        charges_keys = ["Business Pay Bill Charge", "Business Buy Goods Charge", "B2C Payment Charge"]
        dataframe = df.copy()
        mask = dataframe[details_col].str.contains('|'.join(map(re.escape, charges_keys)), case=False, na=False)

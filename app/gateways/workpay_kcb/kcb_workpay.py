...
class KCBWorkpay:
    ...
    def read_file(self):
        ...
        kcb_workpay_dataframe = pd.DataFrame()
            ...
            for file in self.uploads_dir.iterdir():
                ...
                if file.name.startswith(self.prefix):
                    if file.suffix == Configs.EXCEL_SUFFIX:
                        kcb_workpay_dataframe= read_excel_files(file, sheet_name=self.sheet_name, engine=xlsx_engine)
                        break
                    elif file.suffix == Configs.OLD_EXCEL_SUFFIX:
                        kcb_workpay_dataframe = read_excel_files(file, sheet_name=self.sheet_name, engine=xls_engine)
                        break
                    elif file.suffix == Configs.CSV_SUFFIX:
                        kcb_workpay_dataframe = pd.read_csv(file)
                        break
                    else:
                        raise HTTPException(status_code=400, detail="file type not supported")

            if kcb_workpay_dataframe.empty:
                raise HTTPException(status_code=400, detail=f"{self.prefix} workpay dataframe is empty")

            # print(kcb_workpay_dataframe)
            return kcb_workpay_dataframe
    ...
    def clean_data(self):
        ...
        try:
            ...
            dropped_last_row_df_copy.loc[:, 'PAYMENT METHOD'] = (dropped_last_row_df_copy['PAYMENT METHOD'].fillna(random_str))

            mask = dropped_last_row_df_copy['Transaction ID'].astype(str).str.endswith(("-R", "-FR"))
            dropped_last_row_df_copy = dropped_last_row_df_copy[~mask]

            kcb_workpay = pd.DataFrame(columns=self.gateway_cols)
            kcb_workpay[["date", "reference", "method", "debit", "sender", "recipient"]] = (
                dropped_last_row_df_copy[['DATE', 'API Reference', 'PAYMENT METHOD', 'AMOUNT', 'SENDER FEE', 'RECIPIENT FEE']])
            kcb_workpay['status'] = 'Unreconciled'

            # print(kcb_workpay.shape)
            return kcb_workpay
        except Exception as e:
            raise HTTPException(status_code=400, detail=str(e))

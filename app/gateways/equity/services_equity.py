...
def get_equity_data(session_key: str, configs:dict) -> pd.DataFrame:
    try:
...
def clean_equity_data(session_key: str, configs: dict,  gateway_columns:dict) -> pd.DataFrame:
    try:
        ...
        df = drop_bottom_rows(df)

        df = drop_unnamed_columns(df, unnamed_columns)
...
def get_equity_debits(df: pd.DataFrame, details_col: str, debits_col: str, reference_col: str) -> pd.DataFrame:
    try:
        charges_keys = ["JENGA CHARGE", "EFT Comm"]
        pesalink_filter = "TPG"
        jenga_filter = "IFT"
        ...
        for idx,ref in debits_dataframe[details_col].items():
            if not isinstance(ref, str):
                continue
            ref = ref.strip()
            if ref.startswith(pesalink_filter):
                parts = ref.split("/")
                pesalink_ref = parts[-2].strip() if len(parts) >= 2 else ref
                debits_dataframe.loc[idx, reference_col] = pesalink_ref
            elif ref.startswith(jenga_filter):
                jenga_ref = ref.split("_")[0].replace(jenga_filter, "").strip()
                debits_dataframe.loc[idx, reference_col] = jenga_ref
...
def get_equity_credits(df: pd.DataFrame, credits_col: str, remarks_col: str) -> pd.DataFrame:
    try:
        dataframe = df
...
def get_equity_charges(df:pd.DataFrame, details_col: str, credits_col:str, remarks_col:str) -> pd.DataFrame:
    try:
        charges_keys = ["JENGA CHARGE", "EFT Comm"]
        dataframe = df.copy()
        mask = dataframe[details_col].str.contains('|'.join(map(re.escape, charges_keys)), case=False, na=False)

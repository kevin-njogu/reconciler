...
def get_workpay_equity_data(session_key: str, configs:dict) -> pd.DataFrame:
    try:
...
def clean_workpay_equity_data(session_id: str, cols: dict, configs: dict) -> pd.DataFrame:
    try:
        ...
        dataframe = get_workpay_equity_data(session_id, configs)
...
def get_workpay_equity_payouts(df: pd.DataFrame, api_ref_col:str) -> pd.DataFrame:
    try:
        df = df.copy()
...
def get_workpay_equity_refunds(df: pd.DataFrame, ref_col:str, remarks_col:str) -> pd.DataFrame:
    try:
        df = df.copy()
...
def get_workpay_equity_top_ups(df: pd.DataFrame, ref_col:str, remarks_col:str) -> pd.DataFrame:
    try:
        df = df.copy()
        wp_top_ups = df.loc[df[ref_col] == TOP_UP_FILL_KEY]
        wp_top_ups.loc[:, remarks_col] = TOP_UP_FILL_KEY
        return wp_top_ups
    except KeyError as e:
        raise FileOperationsException(f"Invalid column: {e}")
    except Exception:

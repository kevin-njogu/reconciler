...
export function Card({ className, children, ...props }: CardProps) {
  return (
    <div
      className={cn('bg-white rounded-xl shadow-sm border border-neutral-200', className)}
      {...props}
    >
      {children}
    </div>
...
export function CardFooter({ className, children, ...props }: CardFooterProps) {
  return (
    <div className={cn('px-6 py-4 border-t border-neutral-200 bg-neutral-50 rounded-b-xl', className)} {...props}>
      {children}
    </div>
  );
}

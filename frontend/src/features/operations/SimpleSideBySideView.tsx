...
interface SimpleTransactionPanelProps {
  title: string;
...
function SimpleTransactionPanel({
  title,
  transactions,
  selectedIds,
  onToggleSelection,
    ...
    <Card className={cn('flex flex-col h-full', borderColor, 'border-2')}>
      ...
      <CardContent className="p-0 flex-1 overflow-auto">
        <Table>
          ...
          <TableBody>
            {paginatedTransactions.length === 0 ? (
              <TableEmpty message="No unreconciled transactions" colSpan={5} />
            ) : (
...
interface SimpleSideBySideViewProps {
...
export function SimpleSideBySideView({
  byGateway,
  selectedGateway,
  selectedIds,
  onToggleSelection,
  ...
  onToggleAllExternal,
  onReconcileClick,
}: SimpleSideBySideViewProps) {
  // Filter transactions for the selected base gateway
